"""
Phase 6 Test: buyer.created Cross-ERP Synchronization (Yinglima -> Inhyma).

Tests the actual pilot this phase implements: a real Buyer record
created in Inhyma from a Yinglima buyer.created (v2) event, resolving
country_code to Inhyma's own local country_id, with the documented
...
class TestBuyerCreatedSync:
    ...
    async def test_skips_sync_when_country_code_is_missing(self, db_session):
        ...
        await handle_buyer_created({"buyer_id": str(uuid.uuid4()), "company_name": company_name}, "yinglima")
    ...
    async def test_skips_sync_when_country_code_does_not_resolve(self, db_session):
        ...
        await handle_buyer_created(
            {"buyer_id": str(uuid.uuid4()), "company_name": company_name, "country_code": "ZZ_NOT_REAL"}, "yinglima"
        )
    ...
    async def test_skips_sync_when_company_name_missing(self, db_session, seeded_country):
        ...
        await handle_buyer_created({"buyer_id": str(uuid.uuid4()), "country_code": seeded_country.code}, "yinglima")
