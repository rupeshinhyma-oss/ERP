"""
Phase 9 Realtime & Idempotency Hardening Tests for Inhyma ERP.
...
class TestPhase9CredentialHardeningAndFederation:
    ...
    async def test_valid_primary_service_credential_accepted(self, monkeypatch):
        ...
        monkeypatch.setattr(settings, "ERP_MAIN_SERVICE_CREDENTIAL", "inhyma-primary-cred-123")
        monkeypatch.setattr(settings, "ERP_MAIN_SERVICE_CREDENTIAL_PREVIOUS", None)

        creds = HTTPAuthorizationCredentials(scheme="Bearer", credentials="inhyma-primary-cred-123")
        # Should not raise
