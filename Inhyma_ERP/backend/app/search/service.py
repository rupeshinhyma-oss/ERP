...
async def search_universal(db: AsyncSession, query_str: str) -> UniversalSearchResponse:
    ...
    # 12. Tasks (searching title, description, labels, and comments)
    try:
        from app.tasks.models import Task, TaskComment, TaskLabel, TaskLabelLink
        stmt_tasks = (
            select(Task)
            .where(
                ...
                or_(
                    ...
                    Task.id.in_(
                        ...
                        .where(TaskLabel.name.ilike(pattern))
                    ),
                    Task.id.in_(
                        select(TaskComment.task_id)
                        .where(TaskComment.message.ilike(pattern))
                    ),
                ),
            )
        ...
        task_rows = (await db.execute(stmt_tasks)).scalars().all()
        for t in task_rows:
            st = getattr(t.status, "value", str(t.status))
            pr = getattr(t.priority, "value", str(t.priority))
            itype = getattr(t, "issue_type", "TASK")
            results.append(
                SearchResultItem(
                    category="Tasks",
                    id=str(t.id),
                    title=f"[{itype}] {t.title}",
                    subtitle=f"Status: {st} | Priority: {pr}",
                    target_url=f"./tasks.html?id={t.id}",
                    icon="checkSquare",
                )
            )
    except Exception as e:
