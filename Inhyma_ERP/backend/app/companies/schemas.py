"""Company Pydantic Schemas (request/response contracts)."""
...
from app.companies.validators import validate_phone_number
...
class CompanyContactCreate(BaseModel):
    """Payload to add a contact person to a company."""
...
# ---------------------------------------------------------------------------
# Company (First Data Form + Second Form combined)
# ---------------------------------------------------------------------------


class CompanyCreate(BaseModel):
    """Payload to create a new company profile."""
    ...
    contact_indiamart_number: str | None = Field(default=None, max_length=50)
    ...
    product_ids: list[uuid.UUID] = Field(
        default_factory=list, description="Specific Products this company supplies (multiple)."
    )
    ...
    @field_validator("company_type", "company_grade", "current_status", "potential", mode="before")
    @classmethod
...
class CompanyUpdate(BaseModel):
    """Payload to update an existing company profile. All fields optional (partial update)."""
...
class CompanyGradeUpdate(BaseModel):
    """Payload for the list-view inline "editable dropdown" for Grade."""

    company_grade: CompanyGrade | None = None


class CompanyPotentialUpdate(BaseModel):
...
class CompanyRead(BaseModel):
    """A company, as returned by the API (full detail view)."""
...
class CompanyListItemRead(BaseModel):
    """A company, as returned in the list view."""
