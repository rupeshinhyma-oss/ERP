...
export function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    // Optional lightweight state echo: if a previous attempt on this
    // tab recorded a state and this one doesn't match, something is
    // wrong (e.g. a stale/replayed link) -- surface it rather than
    // silently proceeding. This is a UX sanity check, not the
    // security boundary; the actual security guarantees (single-use,
    // short-lived, redirect_uri-bound code, verified server-to-server
    // with this ERP's own client_secret) are enforced by the backend
    // regardless of what this page does with `state`.
    if (state) {
      try {
