/**
 * Error Classification (Phase 2, Section 15).
 *
 * A finer-grained category on top of `retryPolicy.ts`'s TEMPORARY/
 * PERMANENT split, for two purposes `retryPolicy.ts` alone doesn't
 * serve: deciding what a UI should actually SHOW the user (Section 15:
 * "temporary errors should be recoverable [silently]... permanent
 * errors must remain visible and actionable... do not show raw
 * technical stack traces"), and giving diagnostics (Section 16)
 * something more specific than a boolean to record.
 *
 * Every category here maps to exactly one of retryPolicy's two buckets
 * (see `classifyFailure` there) -- this module refines WITHIN that
 * split, it never contradicts it.
 */
