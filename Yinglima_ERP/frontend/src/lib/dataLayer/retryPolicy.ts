/**
 * Retry Policy (Phase 2, Section 8).
 *
 ...
 * way (Section 18) -- the existing frontend retry layer already drew
 * this exact line for its own automatic GET retries; this module
 * applies the same line to queued mutations instead of duplicating the
 * judgment call.
 */
...
/** Statuses considered temporary/worth retrying -- identical set to lib/api.ts's own RETRYABLE_STATUSES, kept in sync deliberately (see module docstring). */
 ...
 * (conflict), 422 (validation) -- is PERMANENT: retrying an identical
 * payload against a business-rule rejection would just fail identically
 * forever (Section 8: "do not create infinite retry loops").
 */
 ...
 *
 * Exponential backoff with +/-20% jitter (same rationale as
 * `lib/live/liveClient.ts`'s own reconnect backoff: many tabs/operations
 * failing at the same moment shouldn't all retry in lockstep), capped at
 * `MAX_DELAY_MS` so a long-failing operation settles into a steady retry
 * cadence rather than growing the delay forever.
 */
