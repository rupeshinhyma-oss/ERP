/**
 * Outgoing Operation Queue (Phase 2, Section 3).
 *
 * The durable record of every mutation a module has asked the data
 * layer to perform on its behalf. Nothing in this file talks to the
 * network -- enqueueing just persists a row; `sync.ts` is what actually
 * sends it. That split is deliberate: a component can call
 * `enqueueOperation(...)` and return immediately (Section 6 -- "instead
 * of throwing a generic error, store it safely"), regardless of whether
 * the network happens to be up at that exact moment.
 *
 * State machine (Section 3's required states):
 *
 ...
 *
 * FAILED entries are moved out of `pending_operations` into
 * `failed_operations` (see `moveToFailed` below) so the active queue
 * `sync.ts` scans on every run stays small (Section 21) -- a
 * permanently-failed entry is never re-examined and skipped on every
...
/**
 * Everything the Sync Manager needs to actually perform this operation
 * later, plus everything a UI needs to display "3 changes pending".
 * Matches Section 3's required field list.
 */
 ...
 * the write to IndexedDB completes -- this function never touches the
...
/** Schedule a retry for a temporarily-failed operation. See retryPolicy.ts for how `nextRetryAt` and `retryCount` are computed. */
 ...
 * active queue and into `failed_operations` (Section 3/21 -- keeps the
 * active queue small; Section 15 -- permanent errors must remain
 * visible, which `getFailedOperations` below serves).
 */
...
/** Cancel a still-pending operation (e.g. the user navigated away and the change is no longer wanted). Never cancels one already PROCESSING. */
...
/** Total count of PENDING + PROCESSING + RETRYING operations -- for UI badges ("3 changes pending") and diagnostics (Section 16). */
