/**
 * Yinglima Data Layer Test Suite: Buyer Pilot (Phase 8C / Phase 9).
 *
...
vi.mock("@/lib/auth", () => {
  const profile = { id: "yinglima-pilot-user" };
  return {
    Auth: {
      getProfile: () => profile,
      subscribe: () => () => {},
...
beforeEach(async () => {
  closeDatabase();
  configureDataLayer("yinglima_buyer_pilot_test");
  await wipeAllStores();
  vi.clearAllMocks();
});

describe("Yinglima Buyer Pilot Integration", () => {
