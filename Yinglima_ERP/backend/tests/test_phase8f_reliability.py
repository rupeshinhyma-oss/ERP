"""
Phase 8F Test Suite: Cross-ERP Reliability, Operations, Backpressure & Recovery Hardening.
...
async def test_01_durable_pending_event_survives_restart():
    ...
    async with session_factory2() as s2:
        ...
        assert recovered.deliveries[0].target_erp == "inhyma"
...
async def test_14_erp_main_outage_independence(db_session):
    ...
    # Destination URL resolves directly to inhyma peer endpoint, not ERP_Main
    dest_url = svc._resolve_target_url(target_erp)
    assert "internal/integration" not in dest_url
    assert "inhyma" in dest_url or "8002" in dest_url
...
async def test_18_fan_out_independent_delivery(db_session):
    ...
    # Publish broadcast event (resolves to inhyma and erp_c)
    ev = svc.publish_event(
        event_type="buyer.created",
        aggregate_type="buyer",
        aggregate_id=uuid.uuid4(),
    ...
    event_id = ev.id
    d_inhyma_id = ev.deliveries[0].id
    await db_session.commit()
    ...
    # Mock inhyma succeeding (200) and erp_c failing (500)
    async def mock_post_fanout(url, *args, **kwargs):
        if "inhyma" in url or "8002" in url:
    ...
    with patch("httpx.AsyncClient.post", side_effect=mock_post_fanout):
        await svc.dispatch_delivery(d_inhyma_id)
        await svc.dispatch_delivery(d_erpc_id)

    d_inhyma = await repo.get_delivery_by_id(d_inhyma_id)
    d_erpc = await repo.get_delivery_by_id(d_erpc_id)

    assert d_inhyma is not None
    assert d_erpc is not None
    assert d_inhyma.status == DeliveryStatus.DELIVERED
...
async def test_21_snapshot_resumable_checkpoint(db_session):
    ...
    job = await svc.start_snapshot_job("buyer", "inhyma", batch_size=2)
...
async def test_25_reconciliation_after_recovery(db_session):
    ...
    report = await rec_svc.reconcile_entity("buyer", source_erp_id="inhyma")
