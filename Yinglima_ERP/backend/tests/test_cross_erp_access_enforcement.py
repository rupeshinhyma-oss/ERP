"""
Cross-ERP Access Enforcement Tests (Yinglima Spoke ERP).

Phase 6: Verifies that central ERP_Main identity and membership authority
is enforced during direct local login when configured, and during federated SSO,
while preserving local RBAC and local authentication integrity.

Tests cover:
1. erp_main_client.lookup_membership_by_local_user (success, 404, network error).
...
4. Federated SSO session issuance:
   - Active local user -> Access and refresh tokens issued.
   - Suspended/inactive local user -> Denied (spoke local authority).
"""
...
async def test_lookup_by_local_user_success():
    ...
    with _patched_client(fake):
        result = await client_module.lookup_membership_by_local_user("yinglima-loc-1")
    assert result["local_user_id"] == "yinglima-loc-1"
...
async def test_lookup_by_local_user_network_error_raises():
    with _patched_client(raise_exc=httpx.ConnectError("Connection failed")):
        with pytest.raises(client_module.MembershipLookupError):
            await client_module.lookup_membership_by_local_user("loc-1")
...
def _create_auth_service(user: User):
    ...
    svc.get_user_effective_permissions = AsyncMock(return_value={"catalog.read"})
    return svc
...
async def test_direct_login_offline_when_enforcement_disabled():
    ...
    with patch.object(settings, "ENFORCE_CENTRAL_MEMBERSHIP_ON_LOGIN", False):
        with patch("app.federation.erp_main_client.lookup_membership_by_local_user") as mock_lookup:
            res_user, access, refresh = await svc.login(
...
# ---------------------------------------------------------------------------
# 3. Federated SSO Session Issuance
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_issue_session_for_federated_user_success():
    """Active local user successfully receives access and refresh tokens upon SSO."""
    user = User(
        id=uuid.uuid4(),
        username="feduser",
        email="fed@yinglima.test",
        status=UserStatus.ACTIVE,
        is_active=True,
        has_login=True,
    )
    svc = _create_auth_service(user)
    access, refresh = await svc.issue_session_for_federated_user(user, LoginContext(ip_address="127.0.0.1"))
    assert access and isinstance(access, str)
    assert refresh and isinstance(refresh, str)


@pytest.mark.asyncio
async def test_issue_session_for_federated_user_denied_when_local_inactive():
    """Locally inactive or suspended user cannot obtain SSO session."""
    user = User(
        id=uuid.uuid4(),
        username="feduser",
        email="fed@yinglima.test",
    ...
    svc = _create_auth_service(user)
    with pytest.raises(UnauthorizedException, match="This account is not active."):
        await svc.issue_session_for_federated_user(user, LoginContext(ip_address="127.0.0.1"))
